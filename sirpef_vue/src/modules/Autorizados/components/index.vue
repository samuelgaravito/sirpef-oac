...
onMounted(() => {
  if(props.persona) UserInfo.value.empleado_id = props.persona.id
})
...
<div class="wrapper-stepper cursor-default fadeout">
    ...
    <div class="stepper-content">
      <p class="text-center my-10">Autorizado para: <strong>{{ persona.nombre_completo }}</strong></p>

      <Formdatabasic :getAuth="getAuth" :emitForm="emitForm" :values="UserInfo" :step="step" v-if="step == 1"/>
      <FormLoadReacudos :emitForm="emitForm" :values="UserInfo" :step="step" :id="persona.id" v-else-if="step == 2"/>
