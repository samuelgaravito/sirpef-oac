...
function renderDeleteButton(data, type, row) {
  if (type === 'display') {
    return `
    <button class="bg-[#ECA008] rounded-lg text-white px-4 py-1 transition-all hover:bg-[#010c41] hover:px-5 btnTable" title="editar ente" onclick='FuncEdit(${row.id})'>
...
onMounted(() => {
  (document as any).FuncEdit = (id: number) => {props.editFunc(id)}
...
onUnmounted(() => {
  ...
  delete (document as any).FuncEdit;
});
