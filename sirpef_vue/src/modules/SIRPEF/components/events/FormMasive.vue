...
const frase = ref("Inserte un archivo")
...
const props = defineProps<{
    senddata: (event: FormData) => Promise<void>,
}>()

const SendFile = async (e: FormDataEvent) => {
        const form = new FormData(e.currentTarget as HTMLFormElement)
        const { name }: any = form.get("file")
...
<template>
      <form class="p-5 border shadow col-span-2 fadeout w-full bg-white rounded-2xl" @submit.prevent="SendFile">
            <div class="p-5 relative border-4 border-dotted border-gray-300 rounded-lg bg-white my-3">
                ...
                <div class="input_field flex flex-col w-max mx-auto text-center">
                    <label>
                        <input class="text-sm cursor-pointer w-36 hidden" name="file" type="file" accept=".csv"
                            @change="changueName" />
                        <div
            ...
            <div class="flex gap-6" v-else>
                ...
                <select name="tipo" class="w-[30%]" required>
                    <option value="empleado">Empleados</option>
                    <option value="fe-de-vida">Jubilados</option>
                </select>
            </div>
        </form>
</template>