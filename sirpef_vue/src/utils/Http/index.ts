...
export class Http {
  ...
  constructor( init: Init ) {    
    ...
    const filterTokenCookie = (config: any) => {
      let token = '';
      let tokenSVA = '';
      try {
        tokenSVA = document.cookie.split('; ').find(row => row.startsWith('SVA-TOKEN='));
        token = localStorage.getItem('SIRPEF-TOKEN');
    
        if (tokenSVA) tokenSVA = tokenSVA.split('=')[1];
      } catch (error) {
        console.error('Error al extraer cookies:', error);
      }
    
      if (!tokenSVA) return config;
        config.headers['svatoken'] = `Bearer ${tokenSVA}`;
    ...
    service.interceptors.request.use(filterTokenCookie);
