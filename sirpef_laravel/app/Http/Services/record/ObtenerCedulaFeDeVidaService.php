...
class ObtenerCedulaFeDeVidaService
{
    public static function ObtenerCedulaFeDeVida($cedula)
    {
        // Obtener el usuario autenticado
        $user = auth()->user();


        $person = Persona::where('cedula', $cedula)->first();

        $unidad = $person->ministerio_id;

        $user->configUser->unid_activa = json_encode([$unidad], true);
